...
from datasets.atari import AtariDataset, AtariDataset2, get_atari_transform
...
class NCSNRunner():
    ...
    def train(self):
        ...
        if self.config.data.dataset.upper() == 'FFHQ':
            ...
            total_frames = self.condf + self.config.data.num_frames + self.futrf
            dataset = AtariDataset(os.path.join(self.args.data_path, "train"), total_frames, transforms=get_atari_transform(self.config.data.image_size), episode_end_frames=self.config.episode_end)
            print(len(dataset))
            ...
            test_dataset = AtariDataset(os.path.join(self.args.data_path, "val"), total_frames, transforms=get_atari_transform(self.config.data.image_size), episode_end_frames=self.config.episode_end)
            print(len(test_dataset))
            test_loader = DataLoader(test_dataset, batch_size=self.config.training.batch_size, shuffle=True, num_workers=self.config.data.num_workers)
            print("val dataloader done")
            test_iter = iter(test_loader)
            print("iterable val dataloader done")
        ...
        if self.args.resume_training:
            states = torch.load(self.args.ckpt_path)
        ...
        # logging sigmas
        hook, test_hook, tb_hook, test_tb_hook = get_sigma_hooks(self.config)
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            for batch, (X, y) in enumerate(dataloader):
                ...
                if self.config.use_encoding:
                    y = convert_encodings(y, self.config.encoding_type, X.shape)
                ...
                X, cond, cond_mask = conditioning_fn(self.config, X, num_frames_pred=self.config.data.num_frames,
                                                     ...
                                                     conditional=conditional, encodings=y if self.config.use_encoding else None)
                ...
                self.time_train.update(convert_time_stamp_to_hrs(str(datetime.timedelta(seconds=self.total_train_time))) + self.time_train_prev)
                ...
                if step == 1 or step % self.config.training.val_freq == 0:
                    ...
                    if self.config.use_encoding:
                        test_y = convert_encodings(test_y, self.config.encoding_type, test_X.shape)
                    ...
                    test_X, test_cond, test_cond_mask = conditioning_fn(self.config, test_X, num_frames_pred=self.config.data.num_frames,
                                                                        prob_mask_cond=getattr(self.config.data, 'prob_mask_cond', 0.0),
                                                                        prob_mask_future=getattr(self.config.data, 'prob_mask_future', 0.0),
                                                                        conditional=conditional, encodings=test_y if self.config.use_encoding else None)
                    ...
                    try:
                        ...
                        pass
                    plot_graphs_process = Process(target=plot_graphs, args=(self.args, self.losses_train, self.losses_test, self.epochs, self.lr_meter, self.grad_norm, self.time_train, self.time_elapsed))
                ...
                self.time_elapsed.update(convert_time_stamp_to_hrs(str(datetime.timedelta(seconds=(time.time() - self.start_time)))) + self.time_elapsed_prev)
    ...
    ### Plotting Video graphs during training
    def plot_video_graphs(self):
        # Pred
        if self.mode_pred is not None:
            plot_video_graphs_single(self.args, "pred",
        ...
        # Interp
        if self.mode_interp is not None:
            plot_video_graphs_single(self.args, "interp",
        ...
        # Gen
        if self.mode_gen is not None and self.calc_fvd3:
            plot_fvd_gen(self.args, self.fvds3, self.best_fvd3)
    ...
    def sample(self):
        ...
        states = torch.load(os.path.join(self.args.ckpt_path), map_location=self.config.device)
        ckpt = 0
        ...
        if not self.config.sampling.fid:
            if self.config.sampling.inpainting:
                ...
                refer_images, refer_encodings = next(data_iter)
                refer_images = refer_images.to(self.config.device)
                refer_images = data_transform(self.config, refer_images)

                if self.config.use_encoding:
                    refer_encodings = convert_encodings(refer_encodings, self.config.encoding_type, refer_images.shape)
                    refer_encodings = refer_encodings.to(self.config.device)

                refer_images, cond, _ = conditioning_fn(self.config, refer_images, num_frames_pred=self.config.data.num_frames, conditional=conditional, encodings=refer_encodings if self.config.use_encoding else None)
                ...
                if self.config.sampling.data_init or conditional:
                    data_iter = iter(dataloader)
                    samples, encodings = next(data_iter)
                    samples = samples.to(self.config.device)
                    samples = data_transform(self.config, samples)
                    if self.config.use_encoding:
                        encodings = convert_encodings(encodings, self.config.encoding_type, samples.shape)
                        encodings = encodings.to(self.config.device)
                    samples, cond, _ = conditioning_fn(self.config, samples, num_frames_pred=self.config.data.num_frames, conditional=conditional, encodings=encodings if self.config.use_encoding else None)
                ...
                if self.config.sampling.data_init or conditional:
                    data_iter = iter(dataloader)
                    real, encodings = next(data_iter)
                    real = real.to(self.config.device)
                    real = data_transform(self.config, real)
                    if self.config.use_encoding:
                        encodings = convert_encodings(encodings, self.config.encoding_type, real.shape)
                        encodings = encodings.to(self.config.device)
                    real, cond, cond_mask = conditioning_fn(self.config, real, num_frames_pred=self.config.data.num_frames, conditional=conditional, encodings=encodings if self.config.use_encoding else None)
                ...
                if conditional:
                    index = self.config.data.channels*(self.config.data.num_frames_cond + self.config.data.num_frames_future)
                    cond = cond[:, :index]
            ...
            for i in tqdm(range(n_rounds), desc='Generating samples for FID'):
                ...
                if self.config.sampling.data_init:
                    ...
                    if self.config.use_encoding:
                        encodings = convert_encodings(encodings, self.config.encoding_type, real.shape)
                        encodings = encodings.to(self.config.device)
                    real, cond, _ = conditioning_fn(self.config, real, num_frames_pred=self.config.data.num_frames, conditional=conditional, encodings=encodings if self.config.use_encoding else None)
                    if self.version == "SMLD":
                        z = torch.randn_like(real)
                        init_samples = real + float(self.config.model.sigma_begin) * z
                    elif self.version == "DDPM" or self.version == "DDIM" or self.version == "FPNDM":
                ...
                if conditional and not self.config.sampling.data_init:
                    try:
                        real, encodings = next(data_iter_cond)
                    except StopIteration:
                        if self.config.data.dataset.upper() == 'FFHQ':
                            dataloader = FFHQ_TFRecordsDataLoader([self.args.data_path], self.config.sampling.batch_size, self.config.data.image_size)
                    ...
                    if self.config.use_encoding:
                        encodings = convert_encodings(encodings, self.config.encoding_type, real.shape)
                        encodings = encodings.to(self.config.device)
                    _, cond, _ = conditioning_fn(self.config, real, num_frames_pred=self.config.data.num_frames, conditional=conditional, encodings=encodings if self.config.use_encoding else None)
            ...
            write_to_pickle(os.path.join(self.args.image_folder, 'fids.pickle'), fids)
            write_to_yaml(os.path.join(self.args.image_folder, 'fids.yml'), fids)
            write_to_pickle(os.path.join(self.args.image_folder, f'precisions_k{k}.pickle'), precisions)
            write_to_yaml(os.path.join(self.args.image_folder, f'precisions_k{k}.yml'), precisions)
            write_to_pickle(os.path.join(self.args.image_folder, f'recalls_k{k}.pickle'), recalls)
            write_to_yaml(os.path.join(self.args.image_folder, f'recalls_k{k}.yml'), recalls)
    ...
    def video_gen(self, scorenet=None, ckpt=None, train=False):
        ...
        for i, (real_, encodings_) in tqdm(enumerate(dataloader), total=min(max_data_iter, len(dataloader)), desc="\nvideo_gen dataloader"):

            if i >= max_data_iter: # stop early
                break

            real_ = data_transform(self.config, real_)
            encodings_ = convert_encodings(encodings_, self.config.encoding_type, real_.shape)
            ...
            real, cond, cond_mask = conditioning_fn(self.config, real_, num_frames_pred=num_frames_pred,
                                                    prob_mask_cond=0.0, prob_mask_future=0.0, conditional=conditional, encodings=encodings_ if self.config.use_encoding else None)
            ...
            if self.config.sampling.data_init:
                ...
                init_samples = z # init_samples is noise of size (batch_size, 3*5, size, size)
            ...
            for i_frame in tqdm(range(n_iter_frames), desc="Generating video frames"):
                ...
                gen_samples = gen_samples[-1].reshape(gen_samples[-1].shape[0], self.config.data.channels*self.config.data.num_frames,
                                                      self.config.data.image_size, self.config.data.image_size) # (batch_size, 3*5, size, size)
                pred_samples.append(gen_samples.to('cpu'))

                if i_frame == n_iter_frames - 1: # last frame
                    continue
                # Will not cross here if not one_frame_at_a_time!!!
                ...
                if cond is None: # first frames are the cond
                    ...
                    cond = torch.cat([cond[:, self.config.data.channels*self.config.data.num_frames:],
                                     ...
                                     ], dim=1) # last 2 frames of gen used in next cond
            ...
            if future > 0 and prob_mask_future > 0.0 and not self.prob_mask_sync:
                ...
                real2, cond, cond_mask = conditioning_fn(self.config, real_, num_frames_pred=num_frames_pred,
                                                         prob_mask_cond=0.0, prob_mask_future=1.0, conditional=conditional, encodings=encodings_ if self.config.use_encoding else None)
                #print(real2.shape)
                ...
                if self.config.use_encoding:
                    ...
                    cond, cond_emb = torch.split(cond, [split_index, cond.shape[1]-split_index], dim=1)
                ...
                for i_frame in tqdm(range(n_iter_frames), desc="Generating video frames"):
                    ...
                    # Generate samples
                    gen_samples = sampler(init_samples if i_frame==0 or getattr(self.config.sampling, 'init_prev_t', -1) <= 0 else gen_samples,
                                          mynet, cond=final_cond, cond_mask=cond_mask,
                                          n_steps_each=self.config.sampling.n_steps_each, step_lr=self.config.sampling.step_lr,
                                          verbose=True if not train else False, final_only=True, denoise=self.config.sampling.denoise,
                                          subsample_steps=getattr(self.config.sampling, 'subsample', None),
                                          clip_before=getattr(self.config.sampling, 'clip_before', True),
        ...
        for ckpt in tqdm(range(self.config.fast_fid.begin_ckpt, self.config.fast_fid.end_ckpt + 1, getattr(self.config.fast_fid, "freq", 5000)),
            ...
            # Generate samples
                ...
                for i in tqdm(range(num_iters), desc="samples"):
                    ...
                    if conditional:
                        try:
                            samples, encodings = next(data_iter_cond)
                        except StopIteration:
                            if self.config.data.dataset.upper() == 'FFHQ':
                                dataloader_cond = FFHQ_TFRecordsDataLoader([self.args.data_path], self.config.sampling.batch_size, self.config.data.image_size)
                            data_iter_cond = iter(dataloader_cond)
                        ...
                        samples, cond, cond_mask = conditioning_fn(self.config, samples, conditional=conditional, encodings=encodings if self.config.use_encoding else None)
        ...
        for ckpt in tqdm(range(self.config.fast_fid.begin_ckpt, self.config.fast_fid.end_ckpt + 1, self.config.fast_fid.freq),
            ...
            for i in range(num_iters):
                ...
                if conditional:
                    ...
                    samples, cond, _ = conditioning_fn(self.config, samples, conditional=conditional, encodings=encodings if self.config.use_encoding else None)
    ...
    def compare_iris_decoded(self, ckpt=None, train=False):
        ...
        for i, (real_, real_2) in tqdm(enumerate(dataloader), total=min(max_data_iter, len(dataloader)), desc="\nvideo_gen dataloader"):
            ...
            if calc_fvd1 or calc_fvd2 or calc_fvd3:
                ...
                if calc_fvd3:
                    ...
                    pred_uncond, _, _ = conditioning_fn(self.config, real_, num_frames_pred=num_frames_pred,
                                                                    prob_mask_cond=1.0, prob_mask_future=1.0, conditional=conditional)
                    pred_uncond = inverse_data_transform(self.config, pred_uncond)
        ...
        format_p = lambda dd : ", ".join([
                    f"{k}:{v:.4f}" if v is not None and k not in ['ckpt', 'preds_per_test', 'time']
                    else f"{k}:{v:7d}" if v is not None and k == 'ckpt'
                    else f"{k}:{v:3d}" if v is not None and k == 'preds_per_test'
