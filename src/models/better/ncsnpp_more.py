...
class NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    ...
    if self.config.use_encoding:
      self.encoding_frames =  num_frames
    ...
    self.numf = numf = config.model.ngf*self.num_frames if self.is3d else config.model.ngf # We must prevent problems by multiplying by num_frames
    ...
    if self.config.use_encoding:
      modules.append(conv3x3(channels*self.n_frames + self.encoding_frames, nf))
    else:
...
class SPADE_NCSNpp(nn.Module):
  ...
  Frames are SPADE Conditioned. Token encodings are concatenated if self.config.use_encoding is True.
  """
  ...
  def forward(self, x, time_cond, cond=None, cond_mask=None):
    ...
    if cond is not None and self.config.use_encoding:
      split_index = self.channels*self.num_frames_cond
...
class SPADE_NCSNpp2(nn.Module):
  """
  NCSN++ model with SPADE normalization

  Frames are concatenated. Token encodings are SPADE conditioned. self.config.use_encoding must be true.  
  """
  ...
  def __init__(self, config):
    ...
    if resblock_type == 'ddpm':
      ResnetBlock = functools.partial(ResnetBlockDDPM,
                                      ...
                                      pseudo3d=self.pseudo3d,
                                      n_frames=self.num_frames,
                                      num_frames_cond=self.encoding_frames,
                                      cond_ch=self.encoding_frames,
                                      spade_dim=spade_dim,
                                      act3d=True) # Activation here as per https://arxiv.org/abs/1809.04096
    ...
    modules.append(conv3x3(channels*(self.n_frames + self.num_frames_cond), nf))
    hs_c = [nf]
    ...
    modules.append(layerspp.get_act_norm(act=act, act_emb=act, norm='spade', ch=in_ch, is3d=self.is3d, n_frames=self.num_frames, num_frames_cond=self.encoding_frames,
                                         cond_ch=self.encoding_frames, spade_dim=spade_dim, cond_conv=conv3x3, cond_conv1=conv1x1_cond))
    modules.append(conv3x3(in_ch, channels*self.num_frames, init_scale=init_scale))
  ...
  def forward(self, x, time_cond, cond=None, cond_mask=None):
    ...
    if cond is not None and self.config.use_encoding:
      split_index = self.channels*self.num_frames_cond
      cond_frames, cond_emb = torch.split(cond, [split_index, cond.shape[1]-split_index], dim=1)
      x = torch.cat([x, cond_frames], dim=1)
      cond = cond_emb
