...
def get_ppo_color_rollouts(env_name, steps, seed=42, num_processes=1,
    ...
    for step in range(steps // num_processes):
        ...
        entropies.append(dist_entropy.clone())
        obs, reward, done, infos = envs.step(action)
        color_obs, color_reward, color_done, color_infos = color_envs.step(action)
    ...
    return episodes, episode_labels, color_episodes
...
def get_episodes(env_name,
    ...
    elif collect_mode == "pretrained_ppo_color":
        #import wandb
        # List of episodes. Each episode is a list of 160x210 observations
        _, episode_labels, episodes = get_ppo_color_rollouts(env_name=env_name,
                                                   steps=steps,
                                                   seed=seed,
                                                   num_processes=num_processes,
                                                   num_frame_stack=num_frame_stack,
