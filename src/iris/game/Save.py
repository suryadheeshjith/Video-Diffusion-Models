...
class Save:
    def __init__(self, env: Union[gym.Env, WorldModelEnv], keymap_name: str, size: Tuple[int, int], fps: int, verbose: bool, record_mode: bool, save_dir: Path) -> None:
        self.env = env
    ...
    def run(self) -> None:
        ...
        while not should_stop:
            ...
            if self.record_mode:
                ...
                img_tensor = TF.to_tensor(img).cuda()
                # print(img_tensor.min(), img_tensor.max())
                # print(img_tensor.shape)
                encoded_image = self.env.agent.tokenizer.encode(img_tensor, should_preprocess=True).z.detach().cpu()
                encoder_buffer.append(np.array(encoded_image))
            ...
            if do_reset or done:
                ...
                if self.record_mode:
                    self.save_recording(np.stack(episode_buffer), np.stack(encoder_buffer))
                    episode_buffer = []
                    encoder_buffer = []

        pygame.quit()

    def save_recording(self, frames, encodings):
        self.record_dir.mkdir(exist_ok=True, parents=True)
        self.counter += 1
        frames_file = str(self.counter) + '_frames.npy'
        encodings_file = str(self.counter) + '_encodings.npy'

        np.save(self.record_dir / frames_file, frames)
        np.save(self.record_dir / encodings_file, encodings)
