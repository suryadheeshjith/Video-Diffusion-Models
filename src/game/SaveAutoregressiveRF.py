...
class SaveAutoregressiveRF:
    def __init__(self, env: Union[gym.Env, WorldModelEnv, AgentEnv], actor_critic: ActorCritic, keymap_name: str, size: Tuple[int, int], fps: int, verbose: bool, record_mode: bool, save_dir: Path, episode_steps: int) -> None:
        self.env = env
    ...
    def atari_transform(self, img, size=64):
        resized = TF.resize(img, (size,size))
        return torch.reshape(img,(3,size,size))
